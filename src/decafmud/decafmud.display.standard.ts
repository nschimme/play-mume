...
var Display = function(decaf, ui, disp) {
        ...
        this.display.className = 'decafmud display ' + this.decaf.options.set_di
splay.fgclass + '7';
        ...
        addEvent(this._display, 'mousedown', function(e){
                if ( e.which !== 2 || !d.decaf.store.get('ui/middle-click-scroll
',false) ) { return; }
                d.scroll();
        ...
        this.message('<br><a href="https://github.com/MUME/DecafMUD">DecafMUD</a
> v' + DecafMUD.version + ' by Stendec &lt;<a href="mailto:stendec365@gmail.com"
>stendec365@gmail.com</a>&gt;<br>');
...
Display.prototype.processData = function() {
        ...
        while ( data.length > 0 ) {
                ...
                var ind = data.indexOf(ESC); //data.search(splitter); //indexOf(
ESC);
                ...
                // Handle the ANSI code. If readANSI returns false, we don't hav
e the
                // whole code yet, so push the remaining data back on the input
buffer
                // and break
        ...
        span.innerHTML = data.replace(/\n\r?/g,'<br>').replace(/> /g,'>&nbsp;')
                .replace(/ ( +)/g, function(m) { if (m.length ===2) {return ' &n
bsp;';}
...
Display.prototype.truncateLines = function() {
        if ((this.display.clientHeight < (window.innerHeight * this.decaf.option
s.set_display.maxscreens)) || (this.display.children.length < this.decaf.options
.set_display.minelements)) {
                return;
        }
        let height = 0;
        let elems = [];
        let targetHeight = (this.display.clientHeight - (window.innerHeight * th
is.decaf.options.set_display.maxscreens));
        while ((height < targetHeight) && (this.display.children.length > elems.
length)) {
...
Display.prototype.handleAnsiCSI = function(seq) {
        switch(seq.charAt(seq.length-1)) {
                // SGR (Select Graphic Rendition) is first because it's the most
 likely
                // to occur and we don't want to waste time comparing against ot
her
                // possibilities first.
                case 'm':
                        var old = this.state, old_fg = this.c_fg, old_bg = this.
c_bg,
                                old_fnt = this.c_fnt;
                        ...
                        for(var i=0;i < l; i++) {
                                ...
                                else if ( 29 < c && c < 38 ) { // Foreground Col
or
                                        this.c_fg = c - 30; continue; }
                                else if ( 39 < c && c < 48 ) { // Background Col
or
                                        this.c_bg = c - 40; continue; }

                                else if ( c === 0 ) { // Reset
                                        this.state = 0; this.c_fg = 7; this.c_bg
 = 0; this.c_fnt = 0; continue; }
                                else if ( c === 1 ) { // Bright
                                        this.state |= BRIGHT; this.state &= ~FAI
NT; continue; }
                                else if ( c === 2 ) { // Faint
                                        this.state &= ~BRIGHT; this.state |= FAI
NT; }
                                        ...
                                        this.state |= UNDERLINE; this.state &= ~
DBLUNDER; }
                                else if ( c < 7   ) { // Blink
                                        this.state |= BLINK; }
                                else if ( c === 7 ) { // Negative
                                        this.state |= NEGATIVE; }
                                else if ( c === 8 ) { continue; } // Conceal Not
 Supported
                                else if ( c === 9 ) { // Strike
                                        this.state |= STRIKE; }
                                else if ( c < 20 ) { // Font
                                        ...
                                        this.state |= DBLUNDER; this.state &= ~U
NDERLINE; }
                                        ...
                                        this.c_bg = 0; }
                                else if ( 89 < c && c < 98 ) { // Bright Foregro
und Color
                                        this.state |= BRIGHT; this.state &= ~FAI
NT;
                                        this.c_fg = c - 90; }
                                else if ( 99 < c && c < 108 ) { // Bright Backgr
ound Color
                                        this.c_bg = c - 92; }
                        }

                        // Do we need a fresh tag?
                        if ( this.state !== old || old_fg !== this.c_fg || old_b
g !== this.c_bg || old_fnt !== this.c_fnt ) {
                                this.outColor(); }
                        this.readyClear = false;
                        return;
...
it.
                                        while(this.outbuf.length > 0) {
                                                var st = this.outbuf[this.outbuf
.length-1];
                                                if ( st.lastIndexOf('\n') === -1
 ) {
                                                        this.outbuf.pop();
...
length-1] = st.substr(0, st.lastIndexOf('\n') + 1);
                                                        break;
                                                }
                                        }
                                }
                                if(!found) {
                                        ...
                                        while(this.display.childElementCount > 0
                                                ...
                                                var last = this.display.children
[this.display.childElementCount-1];
                                                var html = last.innerHTML;
                                                if ( html.lastIndexOf('<br>') ==
= -1 ) {
                                                        this.display.removeChild
(last);
                                                } else {
                                                        found = true;
                                                        last.innerHTML = html.su
bstr(0, html.lastIndexOf('<br>') + 4);
                                                        break;
                                                }
                                        }
                                }
                        } else if ( mode === 1 ) {
                                this.decaf.debugString('ANSI Sequence ESC ['+seq
+' -- TODO: Mode 1');
                        }
                        return;
        }
        ...
        if ( 'ABCDEFGHJKSTfnsulh'.indexOf(seq.charAt(seq.length-1)) !== -1 ) {
                if (! this.vt100Warning ) {
                        this.decaf.debugString("Notice: This display handler onl
y provides"+
                                " a subset of VT100, and doesn't handle cursor m
ovement commands.");
...
; }
        this.needline = false;
        span.innerHTML += text.replace(/ ( +)/g, function(m) { if (m.length ===2
){return ' &nbsp;';} return ' ' + new Array(m.length-1).join('&nbsp;') + ' '; })
 + '<br>';
        this.shouldScroll();
        this.display.appendChild(span);
        this.doScroll();
...
/** Determine if we should be scrolling to the bottom of the output, and do so
        after a short delay if we should. Otherwise, display an element letting
the
        user know they have have content to read if they scroll down. */
Display.prototype.shouldScroll = function(addTarget) {
        if ( this.willScroll !== undefined || this._display.style.overflowY ===
'hidden' ) { return; }
        this.willScroll = this._display.scrollTop + 1>= (this._display.scrollHei
ght - this._display.offsetHeight);

        // If we aren't scrolling, and the element isn't there, add our scroll h
elper.
        if ( addTarget !== false && this.willScroll === false && !this.scrollTar
get) {
...
Display.prototype.doScroll = function() {
        ...
        if ( this.willScroll ) {
                this.scrollTime = setTimeout(function(){
                        if ( d.scrollTarget ) {
                                d.scrollTarget.parentNode.removeChild(d.scrollTa
rget);
...
Display.prototype.onScroll = function() {
        ...
        if (!(this._display.scrollTop >= (this._display.scrollHeight - this._dis
play.offsetHeight))) {
                return; }
...
Display.prototype.tags = {
        'VAR'           : {
                ...
                'open_tag'      : '',
                'close_tag'     : '',
                'arg_order'     : ['name','desc','private','publish','delete','a
dd','remove'],
                'arguments'     : {
                        'name'          : '',
                        'desc'          : '',
        ...
        'COLOR'         : {
                ...
                'secure'        : false,
                'want'          : false,
                'open_tag'      : '<span class="mxp mxp-color" style="color:&for
e;;background-color:&back;">',
                'close_tag'     : '</span>',
                'arg_order'     : ['fore','back'],
                'arguments'     : {
                        'fore'          : 'inherit',
