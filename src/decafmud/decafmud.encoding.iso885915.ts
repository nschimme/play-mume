...
for(const k in replaces) {
    if (replaces.hasOwnProperty(k)) { // Ensure k is a direct property
        rep_reg_str += k;
...
const decode = function(data: string): [string, string] {
    return [data.replace(rep_reg, function(m: string): string { return replaces[m] || m; }), ''];
};

const encode = function(data: string): string {
    return data.replace(unrep_reg, function(m: string): string { return unreplaces[m] || m; });
};
...
(DecafMUDGlobal as any).plugins.Encoding.iso885915 = {
    proper : 'ISO-8859-15',
