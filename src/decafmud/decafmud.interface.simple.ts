...
var SimpleInterface = function(decaf) {
        ...
        var h = function(){if(!this.className){return;}this.className = this.cla
ssName.replace(' visible','');}
...
SimpleInterface.prototype.toString = function() {
        return '<DecafMUD Interface: Simple' + (this.container.id ? ' (#'+this.c
ontainer.id+')' : '') + '>'; }
...
SimpleInterface.prototype.initSplash = function(percentage,message) {
        ...
        if ( message === undefined ) { message = 'Discombobulating interface rec
ipient...'.tr(this.decaf); }
        ...
        // Build the contents.
        this.splash.innerHTML  = '<h2 class="decafmud heading"><a href="https://
github.com/MUME/DecafMUD">DecafMUD</a> <span class="version">v'+DecafMUD.version
+'</span></h2>';
        ...
        this.splash_pg.setAttribute('aria-valuetext', '{0}%'.tr(this.decaf,perce
ntage));
...
SimpleInterface.prototype.endSplash = function() {
        ...
        this.splash = this.splash_pg = this.splash_pgi = this.splash_pgt = this.
splash_st = null;
}
...
SimpleInterface.prototype.connected = function() {
        this.updateIcon(this.ico_connected, "DecafMUD is currently connected.".t
r(this.decaf),
...
SimpleInterface.prototype.connecting = function() {
        this.updateIcon(this.ico_connected, "DecafMUD is attempting to connect."
.tr(this.decaf),
...
SimpleInterface.prototype.disconnected = function() {
        this.updateIcon(this.ico_connected, "DecafMUD is currently disconnected.
".tr(this.decaf),
...
SimpleInterface.prototype.setup = function() {
        ...
        // Create the display.
        this.decaf.debugString('Initializing display plugin "'+display+'" in: #'
 + this.el_display.id,'info');
        this.display = new DecafMUD.plugins.Display[display](this.decaf, this, t
his.el_display);
        ...
        var fs = this.store.get('fullscreen-start', this.decaf.options.set_inter
face.start_full);
        ...
        this.logbutton = this.tbNew(
                "Logs".tr(this.decaf),
                undefined,
                "Click to open a window containing this session's logs.".tr(this
.decaf),
                0,
                true,
                false,
        ...
        this.ico_connected = this.addIcon("You are currently disconnected.".tr(t
his.decaf), '', 'connectivity disconnected');
        ...
        if ( window.getComputedStyle ) {
                ...
                while(node) {
                        ...
                        if (!style.backgroundColor ||
                                ...
                                css = 'background-color:' + style.backgroundColo
r + ';';
                                break;
                        }
                }
                ...
                css2 = 'div{font-family:'+style.fontFamily+';font-size:'+style.f
ontSize+';}';
        } else {
        ...
        var win = window.open(url,'log-window','width=700,height=400,directories
...
SimpleInterface.prototype.showSettings = function() {
        ...
        if ( this.settings ) {
                ...
                this.tbTooltip(this.stbutton,"Click to change DecafMUD's setting
s.".tr(this.decaf))
                this.el_display.setAttribute('tabIndex','0');
        ...
        d.innerHTML = "Use the form below to adjust DecafMUD's settings, then cl
ick Apply when you're done.".tr(this.decaf);
        cont.appendChild(d);
        ...
        this.tbTooltip(this.stbutton,"Click to close the settings window.".tr(th
is.decaf));
}
...
SimpleInterface.prototype.tbPressed = function(id, pressed) {
        ...
        if ( pressed ) {
                if ( /toggle-depressed/.test(btn[0].className) ) {
                        btn[0].className = btn[0].className.replace(' toggle-dep
ressed',' toggle-pressed'); }
        } else {
                if ( /toggle-pressed/.test(btn[0].className) ) {
                        btn[0].className = btn[0].className.replace(' toggle-pre
ssed',' toggle-depressed'); }
        }
}
...
SimpleInterface.prototype.tbClass = function(id, clss) {
        ...
        var old_clss = btn[7];
        btn[7] = clss;
        if ( old_clss !== undefined ) { btn[0].className = btn[0].className.repl
ace(' '+old_clss,''); }
...
SimpleInterface.prototype.tbIcon = function(id, icon) {
        ...
        btn[2] = icon;
        if ( icon ) {
                if (! / icon/.test(btn[0].className) ) { btn[0].className += ' i
con'; }
 ...
 *    or toggled. */
SimpleInterface.prototype.tbNew = function(text,icon,tooltip,type,enabled,presse
d,clss,onclick) {
        ...
        if ( type === 1 ) { btn.className += ' toggle ' + (pressed ? 'toggle-pre
ssed' : 'toggle-depressed'); }
        btn.innerHTML = text;
...
; }
        ...
        addEvent(btn,'focus',function(e) {
                ...
                if (/toolbar/.test(this.parentNode.className)) {
                        this.parentNode.setAttribute('aria-activedescendant', th
is.id);
        ...
        addEvent(btn,'blur',function(e) {
                if (! this.parentNode ) { return; }
                if (/toolbar/.test(this.parentNode.className)) {
                        if ( this.parentNode.getAttribute('aria-activedescendant
') === this.id ) {
                                this.parentNode.setAttribute('aria-activedescend
ant', ''); }
                        this.parentNode.className = this.parentNode.className.re
place(' visible',''); }
        });

        // Store the button.
        this.toolbuttons[ind] = [btn,text,icon,tooltip,type,enabled,pressed,clss
,onclick];
...
SimpleInterface.prototype._resizeToolbar = function() {
        ...
        if ( this.old_tbarpos !== tbar ) {
                // Move the toolbar.
                this.toolbar.className = this.toolbar.className.replace(' '+this
.old_tbarpos,' '+tbar);

                // Remove the old class from the toolbar's CSS.
                if ( this.old_tbarpos === 'top-left' || this.old_tbarpos === 'to
p-right' ) {
        ...
        } else if ( this.toolbar.className.indexOf(' always-on') !== -1 && !alwa
ys ) {
                this.toolbar.className = this.toolbar.className.replace(' always
-on','');
        }

        // If the toolbar is always-on and top-left or top-right...
        if ( / always-on/.test(this.toolbar.className) && / top-(?:left|right)/.
test(this.toolbar.className) ) {
                if ( this.settings && this.toolbarPadding !== this.toolbar.clien
tHeight ) {
                        this.settings.style.paddingTop = (this.toolbar.clientHei
ght-5) + 'px';
                }
                if ( this.display && this.toolbarPadding !== this.toolbar.client
Height ) {
                        // If we have a display, check its padding.
                        this.display.shouldScroll();
                        this.el_display.style.paddingTop = this.toolbar.clientHe
ight + 'px';
...
SimpleInterface.prototype.infoBar = function(text, clss, timeout, icon, buttons,
 click, close) {
...
SimpleInterface.prototype.immediateInfoBar = function(text, clss, timeout, icon,
 buttons, click, close) {
...
SimpleInterface.prototype.createIBar = function() {
        ...
        var closer = function(e) {
                if ( e === undefined || ( e.type === 'keydown' && e.keyCode !==
13 && e.keyCode !== 27 )) { return; }
...
SimpleInterface.prototype.closeIBar = function(steptwo) {
        ...
        if ( !steptwo ) {
                // Fade it nicely.
                this.ibar.style.cssText += '-webkit-transition: opacity 0.25s li
near;' +
...
SimpleInterface.prototype.addIcon = function(text, html, clss, onclick, onkey) {
        ...
        ico.className = 'decafmud status-icon ' + clss + ( onclick ? ' icon-clic
k' : '' );
        ico.innerHTML = html;
        ...
        for(var i=0; i < this.icons.length; i++) {
                this.icons[i][0].style.cssText = 'right:'+(((this.icons.length-i
)-1)*21)+'px';
        }
...
SimpleInterface.prototype.delIcon = function(ind) {
        ...
        // Recalculate icon positions.
        for(var i=0; i < this.icons.length; i++) {
                this.icons[i].style.cssText = 'right:'+(((this.icons.length-i)-1
)*21)+'px';
        }
...
SimpleInterface.prototype.updateIcon = function(ind, text, html, clss) {
        ...
        if ( clss ) { el.className = 'decafmud status-icon ' + clss + ( onclick
? ' icon-click' : ''); }
...
SimpleInterface.prototype.enter_fs = function(showSize) {
        ...
        if ( this.next_sib === undefined ) {
                ...
                if ( this.container.nextSibling && this.container.nextSibling.no
deType == this.container.nodeType ) {
        ...
        this.tbTooltip(this.fsbutton, "Click to exit fullscreen mode.".tr(this.d
ecaf));

        // Hide all the other body elements.
        for(var i=0;i<document.body.children.length;i++) {
                var child = document.body.children[i];
                if ( child.id !== '_firebugConsole' && child.id.indexOf('DecafFl
ashSocket') !== 0 ) {
...
SimpleInterface.prototype.exit_fs = function() {
        ...
        this.tbTooltip(this.fsbutton, "Click to enter fullscreen mode.".tr(this.
decaf));
...
SimpleInterface.prototype.resizeScreen = function(showSize,force) {
        ...
        if ( this.goFullOnResize ) {
                ...
                if ( !fs ) {
                        if ( window.outerHeight ) { fs = (window.screen.height -
 window.outerHeight) <= 5; }
                        else if ( window.innerHeight ) { fs = (window.screen.hei
ght - window.innerHeight <= 5); }
                }
                if ( fs && !this.old_fs ) {
                        this.old_fs = fs;
                        this.enter_fs();
        ...
        if ( force !== true && this.old_height === this.container.offsetHeight &
& this.old_width === this.container.offsetWidth ) { return; }
...
';
        ...
        if ( this.scrollButton ) {
                this.scrollButton.style.cssText = 'bottom:' + (this._input.offse
tHeight + 12) + 'px';
        }
...
SimpleInterface.prototype.displayKey = function(e) {
        ...
        if (!( (e.keyCode > 64 && e.keyCode < 91) || (e.keyCode > 47 && e.keyCod
e < 58)
                || (e.keyCode > 185 && e.keyCode < 193)||(e.keyCode > 218 && e.k
eyCode < 223) )) {
                return; }
...
SimpleInterface.prototype.updateInput = function(force) {
        ...
        if ( force !== true && ( (!this.echo && type === 'password') || (this.ec
ho && type !== 'password') ) ) {
                return; }
        ...
        if ( pos === undefined ) {
                // Try getting nextSibling for IE support
                if ( inp.nextSibling && inp.nextSibling.nodeType === inp.nodeTyp
e ) {
                        pos = inp.nextSibling; }
        }
        ...
        if ( !this.echo ) {
                ...
                addEvent(new_inp, 'keydown', function(e) { si.handleInputPasswor
d(e); });
                ...
                // If one line, we're dealing with basic input. If more than one
, a
                // textarea.
                if ( lines === 1 ) {
